...
## zrender
`zrender`是`echarts`用的`2d`渲染器，也是`echarts`唯一的依赖。它是一个轻量的二维绘图引擎，但是实现了很多功能。本文就从实现`zrender`开始作为实现`echarts`的第一步，也许也是最重要的一步。
> 这个切入可能有点突兀，也许该先分析`echarts`图表的组成部分，再慢慢切到这个话题。
## 本篇目标
...
### 环境搭建
为了方便，我使用了`vue-cli`搭建环境，你也可以用其它方式，只要能支持出现的语法就行。接着创建`xrender`目录。根据上面列出的类，创建如下文件。
``` sh
index.js # 外部引用的入口
Painter.js 
Stage.js
